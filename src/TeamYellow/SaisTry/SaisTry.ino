...
void loop() {
  // for the first 3 hashes, follow to a hash, then compute a light value
  for (int i = 0; i < 3; i++) {
    lineFollow(-1, 1);
    computeBulb();
  }
  // follow to the 4th hash
  lineFollow(-1, 1);

  // stop moving at the last hash
  Move(0,0);

  // use austin’s communication here
  delay(10000);
